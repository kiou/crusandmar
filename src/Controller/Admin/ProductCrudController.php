...
class ProductCrudController extends AbstractCrudController
{
    ...
    public function configureFields(string $pageName): iterable
    {
         ...
         yield ImageField::new('image','Picture 1')
            ...
            ->setFormTypeOption('upload_new', function(UploadedFile $file) use ($timestamp){
                ...
                $size = new Box(400,350);
                $imagine->open($file)
                        ->thumbnail($size, 'outbound')
                        ->save($this->getUploadRootDir().'miniature/'.$image);
        });
        yield ImageField::new('image2', 'Picture 2')
            ->setUploadDir('public/img/upload/')
            ->setBasePath('img/upload/')
            ->setUploadedFileNamePattern('[slug]-'.$timestamp.'.[extension]')
            ->setFormTypeOption('upload_new', function(UploadedFile $file) use ($timestamp){
        ...
        yield ImageField::new('image3', 'Picture 3')
            ->setUploadDir('public/img/upload/')
            ->setBasePath('img/upload/')
            ->setUploadedFileNamePattern('[slug]-'.$timestamp.'.[extension]')
            ->setFormTypeOption('upload_new', function(UploadedFile $file) use ($timestamp){
        ...
        yield BooleanField::new('isActive','Avtive')->onlyOnIndex();
